...
@interface MKSBOnOffSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBOnOffSettingsModel *dataModel;

@end

@implementation MKSBOnOffSettingsController

- (void)dealloc {
    NSLog(@"MKSBOnOffSettingsController销毁");
}
...
- (void)configOffByButton:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSBInterface sb_configOFFByButton:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configAutoPowerOn:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSBInterface sb_configAutoPowerOn:isOn sucBlock:^{
        [[MKHudManager share] hide];
        MKTextSwitchCellModel *cellModel = self.dataList[3];
        cellModel.isOn = isOn;
        self.dataModel.powerOn = isOn;
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (MKSBOnOffSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBOnOffSettingsModel alloc] init];
    }
    return _dataModel;
}
