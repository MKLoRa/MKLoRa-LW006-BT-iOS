...
@interface MKSBLoRaSettingController ()<UITableViewDelegate,
...
MKSBLoRaSettingAccountCellDelegate>
...
@property (nonatomic, strong)MKSBLoRaSettingModel *dataModel;

@end

@implementation MKSBLoRaSettingController

- (void)dealloc {
    NSLog(@"MKSBLoRaSettingController销毁");
}
...
- (void)rightButtonMethod {
    ...
    if (!ValidStr([MKSBUserLoginManager shared].password)) {
        //当前未登录
        @weakify(self);
        MKIoTCloudAccountLoginAlertViewModel *viewModel = [[MKIoTCloudAccountLoginAlertViewModel alloc] init];
        viewModel.account = [MKSBUserLoginManager shared].username;
        viewModel.isHome = [MKSBUserLoginManager shared].isHome;
        viewModel.password = [MKSBUserLoginManager shared].password;
    ...
    //登录过
    [self login:[MKSBUserLoginManager shared].isHome username:[MKSBUserLoginManager shared].username password:[MKSBUserLoginManager shared].password];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 7) {
        //Account
        return ((self.dataModel.configModel.supportServerPlatform && self.dataModel.platform == 1 && ValidStr([MKSBUserLoginManager shared].password)) ? self.section6List.count : 0);
    }
    if (section == 8) {
        //Gateway EUI(Gateway ID)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        //mokoDevEUI
        MKSBDevEUICell *cell = [MKSBDevEUICell initCellWithTableView:tableView];
    ...
    if (indexPath.section == 7) {
        //Account
        MKSBLoRaSettingAccountCell *cell = [MKSBLoRaSettingAccountCell initCellWithTableView:tableView];
...
#pragma mark - MKSBLoRaSettingAccountCellDelegate
- (void)sb_loRaSettingAccountCell_logoutBtnPressed {
    MKIoTCloudExitAccountAlert *alert = [[MKIoTCloudExitAccountAlert alloc] init];
    BOOL isHome = [MKSBUserLoginManager shared].isHome;
    NSString *username = [MKSBUserLoginManager shared].username;
    [alert showWithAccount:username completeBlock:^{
        [[MKSBUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:@""];
...
- (void)login:(BOOL)isHome username:(NSString *)username password:(NSString *)password {
    ...
    [[MKIoTLoginService share] loginWithUsername:username password:password isHome:isHome sucBlock:^(id returnData) {
        ...
        [[MKSBUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:password];
...
- (void)addDeviceToCloud:(NSString *)token {
    ...
    MKSBCreateLoRaDeviceModel *createModel = [[MKSBCreateLoRaDeviceModel alloc] init];
    createModel.macAddress = [MKSBConnectModel shared].macAddress;
    createModel.isHome = [MKSBUserLoginManager shared].isHome;
    createModel.gwId = self.dataModel.gatewayEUI;
    createModel.region = self.dataModel.region;
    createModel.username = [MKSBUserLoginManager shared].username;
    [[MKSBNetworkService share] addLoRaDeviceToCloud:createModel token:token sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
        [self saveDataToDevice];
    } failBlock:^(NSError *error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKSBDevEUICellModel *cellModel = [[MKSBDevEUICellModel alloc] init];
    NSString *macAddress = [[MKSBConnectModel shared].macAddress stringByReplacingOccurrencesOfString:@":" withString:@""];
...
- (void)loadSection6Datas {
    MKSBLoRaSettingAccountCellModel *cellModel = [[MKSBLoRaSettingAccountCellModel alloc] init];
    cellModel.account = [MKSBUserLoginManager shared].username;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBLoRaSettingController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBLoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBLoRaSettingModel alloc] init];
    }
    return _dataModel;
}
