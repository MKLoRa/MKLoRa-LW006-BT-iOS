...
@interface MKSBLCGpsFixController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBLCGpsFixModel *dataModel;

@end

@implementation MKSBLCGpsFixController

- (void)dealloc {
    NSLog(@"MKSBLCGpsFixController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBLCGpsFixController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBLCGpsFixModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBLCGpsFixModel alloc] init];
    }
    return _dataModel;
}
