...
@implementation MKSBLCGpsFixModel
...
- (BOOL)readPositioningTimeout {
    ...
    [MKSBInterface sb_readLCPositioningTimeoutWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configPositioningTimeout {
    __block BOOL success = NO;
    [MKSBInterface sb_configLCPositioningTimeout:[self.timeout integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readPDOP {
    __block BOOL success = NO;
    [MKSBInterface sb_readLCPDOPWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configPDOP {
    __block BOOL success = NO;
    [MKSBInterface sb_configLCPDOP:[self.pdop integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readExtrmeMode {
    __block BOOL success = NO;
    [MKSBInterface sb_readLCGpsExtrmeModeStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configExtrmeMode {
    __block BOOL success = NO;
    [MKSBInterface sb_configLCGpsExtrmeModeStatus:self.extrmeMode sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
