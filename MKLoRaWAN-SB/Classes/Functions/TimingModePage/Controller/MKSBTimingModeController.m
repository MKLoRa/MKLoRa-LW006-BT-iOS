...
@interface MKSBTimingModeController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKSBTimingModeAddCellDelegate,
MKSBReportTimePointCellDelegate>
...
@property (nonatomic, strong)MKSBTimingModeModel *dataModel;

@end

@implementation MKSBTimingModeController

- (void)dealloc {
    NSLog(@"MKSBTimingModeController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSBTimingModeAddCell *cell = [MKSBTimingModeAddCell initCellWithTableView:tableView];
        cell.dataModel = self.section1List[indexPath.row];
        cell.delegate = self;
        return cell;
    }
    MKSBReportTimePointCell *cell = [MKSBReportTimePointCell initCellWithTableView:tableView];
...
#pragma mark - MKSBTimingModeAddCellDelegate
- (void)sb_addButtonPressed {
    ...
    MKSBReportTimePointCellModel *cellModel = [[MKSBReportTimePointCellModel alloc] init];
...
#pragma mark - MKSBReportTimePointCellDelegate
/**
 删除
...
- (void)sb_cellDeleteButtonPressed:(NSInteger)index {
    ...
    for (NSInteger i = 0; i < self.section2List.count; i ++) {
        MKSBReportTimePointCellModel *cellModel = self.section2List[i];
        cellModel.index = i;
...
- (BOOL)cellCanSelected{
    ...
    for (int i = 0; i < [arrCells count]; i++) {
        ...
        if ([cell isKindOfClass:MKSBReportTimePointCell.class]) {
            MKSBReportTimePointCell *tempCell = (MKSBReportTimePointCell *)cell;
...
- (void)showTimePointHourPickView:(NSInteger)index {
    MKSBReportTimePointCellModel *cellModel = self.section2List[index];
...
- (void)loadSection2Datas {
    for (NSInteger i = 0; i < self.dataModel.pointList.count; i ++) {
        MKSBTimingModeTimePointModel *tempModel = self.dataModel.pointList[i];
        MKSBReportTimePointCellModel *cellModel = [[MKSBReportTimePointCellModel alloc] init];
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"Time Point %ld",(long)(i + 1)];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBTimingModeController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBTimingModeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBTimingModeModel alloc] init];
    }
    return _dataModel;
}
