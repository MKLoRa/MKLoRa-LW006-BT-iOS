...
@implementation MKSBReportTimePointCellModel
@end

@interface MKSBReportTimePointCell ()
...
@implementation MKSBReportTimePointCell

+ (MKSBReportTimePointCell *)initCellWithTableView:(UITableView *)tableView {
    MKSBReportTimePointCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSBReportTimePointCellIdenty"];
    if (!cell) {
        cell = [[MKSBReportTimePointCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSBReportTimePointCellIdenty"];
    }
    return cell;
}
...
- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sb_cellDeleteButtonPressed:)]) {
        [self.delegate sb_cellDeleteButtonPressed:self.dataModel.index];
    }
}
...
- (void)hourButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(sb_hourButtonPressed:)]) {
        [self.delegate sb_hourButtonPressed:self.dataModel.index];
    }
}
...
- (void)timeSpaceButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(sb_timeSpaceButtonPressed:)]) {
        [self.delegate sb_timeSpaceButtonPressed:self.dataModel.index];
    }
}

- (void)contentPanelTapAction {
    if (self.contentPanel.frame.origin.x == 0) {
        if ([self.delegate respondsToSelector:@selector(sb_cellTapAction)]) {
            [self.delegate sb_cellTapAction];
        }
        return;
    }
    if (self.contentPanel.frame.origin.x < 0){
...
- (void)swipeEventBeTriggered:(UISwipeGestureRecognizer *)swipeGesture{
    if ([self.delegate respondsToSelector:@selector(sb_cellResetFrame)]) {
        [self.delegate sb_cellResetFrame];
    }
...
#pragma mark - setter
- (void)setDataModel:(MKSBReportTimePointCellModel *)dataModel {
    _dataModel = nil;
