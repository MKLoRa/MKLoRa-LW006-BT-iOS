...
@interface MKSBDeviceModeController ()<UITableViewDelegate,
...
@implementation MKSBDeviceModeController

- (void)dealloc {
    NSLog(@"MKSBDeviceModeController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Standby Mode
        MKSBStandbyModeModelController *vc = [[MKSBStandbyModeModelController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //Timing Mode
        MKSBTimingModeController *vc = [[MKSBTimingModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Periodic Mode
        MKSBPeriodicModeController *vc = [[MKSBPeriodicModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 3) {
        //Motion Mode
        MKSBMotionModeController *vc = [[MKSBMotionModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)readDataFromDevice {
    ...
    [MKSBInterface sb_readWorkModeWithSucBlock:^(id  _Nonnull returnData) {
...
- (void)saveWorkModeToDevice:(NSInteger)mode {
    ...
    [MKSBInterface sb_configWorkMode:mode sucBlock:^{
        [[MKHudManager share] hide];
