...
@interface MKSBLoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKSBLoRaPageModel *dataModel;

@end

@implementation MKSBLoRaController

- (void)dealloc {
    NSLog(@"MKSBLoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKSBLoRaSettingController *vc = [[MKSBLoRaSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.row == 2) {
        //Application Settings
        MKSBLoRaAppSettingController *vc = [[MKSBLoRaAppSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (MKSBLoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBLoRaPageModel alloc] init];
    }
    return _dataModel;
}
