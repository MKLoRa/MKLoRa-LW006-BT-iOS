...
@interface MKSBSynDataController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_sb_storageDataDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKSBSynTableHeaderView *headerView;
...
@implementation MKSBSynDataController

- (void)dealloc {
    NSLog(@"MKSBSynDataController销毁");
    if (self.dataTimer) {
    ...
    [[MKSBCentralManager shared] notifyStorageData:NO];
}
...
- (void)viewDidLoad {
    ...
    [[MKSBCentralManager shared] notifyStorageData:YES];
    [MKSBCentralManager shared].dataDelegate = self;
}
...
- (void)leftButtonMethod {
    ...
    [MKSBInterface  sb_pauseSendLocalData:YES sucBlock:^{
        [self startBackTimer];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [MKSBSynDataCell fetchCellHeight:self.dataList[indexPath.row]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKSBSynDataCell *cell = [MKSBSynDataCell initCellWithTableView:tableView];
...
#pragma mark - mk_sb_storageDataDelegate
...
- (void)startButtonPressed {
    ...
    [MKSBInterface sb_readNumberOfDaysStoredData:[self.headerView.textField.text integerValue] sucBlock:^{
        self.isMaxCount = ([self.headerView.textField.text integerValue] == 65535);
        [[NSUserDefaults standardUserDefaults] setValue:self.headerView.textField.text forKey:@"sb_readRecordDataDayNumKey"];
...
- (void)synButtonPressed {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSBInterface sb_pauseSendLocalData:!self.headerView.synButton.selected sucBlock:^{
        [[MKHudManager share] hide];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (void)emptyButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSBInterface sb_clearAllDatasWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)readDataFromLocal {
    ...
    [MKSBDatabaseManager readDataListWithSucBlock:^(NSArray<NSDictionary *> * _Nonnull dataList) {
        [[MKHudManager share] hide];
...
- (void)processStatus {
    self.headerView.textField.text = [[NSUserDefaults standardUserDefaults] objectForKey:@"sb_readRecordDataDayNumKey"];
    self.totalSum = [[NSUserDefaults standardUserDefaults] objectForKey:@"sb_recordDataTotalSumKey"];
    ...
    if (self.dataList.count > 0) {
        ...
        self.headerView.synButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_sync_disableIcon.png");
        
        self.headerView.emptyButton.enabled = NO;
        self.headerView.emptyButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_delete_disableIcon.png");
        
        self.headerView.exportButton.enabled = NO;
        self.headerView.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_export_disableIcon.png");
    }
...
- (void)startButtonMethod {
    ...
    self.headerView.synButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_sync_enableIcon.png");
    [self.headerView.synButton.topIcon.layer removeAnimationForKey:synIconAnimationKey];
    //开始旋转
...
- (void)syncButtonStateUpdate {
    ...
    if (self.headerView.synButton.selected) {
        ...
        self.headerView.emptyButton.topIcon.image = (enable ? LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_delete_enableIcon.png") : LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_delete_disableIcon.png"));
        
        self.headerView.exportButton.enabled = enable;
        self.headerView.exportButton.topIcon.image = (enable ? LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_export_enableIcon.png") : LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_export_disableIcon.png"));
...
- (void)emptyButtonMethod {
    ...
    self.headerView.emptyButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_delete_enableIcon.png");
    
    self.headerView.exportButton.enabled = YES;
    self.headerView.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBSynDataController", @"sb_export_enableIcon.png");
}
...
- (void)saveDataToLocal {
    [MKSBDatabaseManager clearDataTable];
    [[NSUserDefaults standardUserDefaults] setValue:SafeStr(self.totalSum) forKey:@"sb_recordDataTotalSumKey"];
    if (self.dataList.count == 0) {
        [[MKHudManager share] hide];
        [super leftButtonMethod];
        return;
    }
    [MKSBDatabaseManager insertDataList:self.dataList sucBlock:^{
        [[MKHudManager share] hide];
        [super leftButtonMethod];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)addDataTimer {
    ...
    dispatch_source_set_event_handler(self.dataTimer, ^{
        @strongify(self);
        moko_dispatch_main_safe(^{
            [MKSBInterface sb_readThreeAxisWakeupConditionsWithSucBlock:nil failedBlock:nil];
        });
    });
...
- (MKSBSynTableHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKSBSynTableHeaderView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 100.f)];
