...
@interface MKSBPositionPageModel ()
...
@implementation MKSBPositionPageModel
...
- (BOOL)readOfflineFix {
    ...
    [MKSBInterface sb_readOfflineFixStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configOfflineFix:(BOOL)offline {
    __block BOOL success = NO;
    [MKSBInterface sb_configOfflineFix:offline sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
