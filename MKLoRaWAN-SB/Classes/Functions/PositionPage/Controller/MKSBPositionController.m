...
@implementation MKSBPositionController

- (void)dealloc {
    NSLog(@"MKSBPositionController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Fix
        MKSBWifiFixController *vc = [[MKSBWifiFixController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Bluetooth Fix
        MKSBBleFixController *vc = [[MKSBBleFixController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        ...
        //LR1110
        MKSBLRGpsFixController *vc = [[MKSBLRGpsFixController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (MKSBPositionPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBPositionPageModel alloc] init];
    }
    return _dataModel;
}
