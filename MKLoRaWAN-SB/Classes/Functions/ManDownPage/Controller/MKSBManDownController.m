...
@interface MKSBManDownController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBManDownDataModel *dataModel;

@end

@implementation MKSBManDownController

- (void)dealloc {
    NSLog(@"MKSBManDownController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBManDownController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBManDownDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBManDownDataModel alloc] init];
    }
    return _dataModel;
}
