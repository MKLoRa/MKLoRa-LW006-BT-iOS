...
@interface MKSBDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_sb_centralManagerLogDelegate,
MKSBDebuggerCellDelegate>
...
@property (nonatomic, strong)MKSBDebuggerButton *deleteButton;

@property (nonatomic, strong)MKSBDebuggerButton *exportButton;
...
@implementation MKSBDebuggerController

- (void)dealloc {
    NSLog(@"MKSBDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_sb_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self loadSubViews];
    [MKSBCentralManager shared].logDelegate = self;
    [self readDataFromLocal];
    //进入debugger模式之后，即使设备断开连接，也要停留在当前页面
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_sb_startDebuggerMode" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceDisconnect)
                                                 name:mk_sb_peripheralConnectStateChangedNotification
                                               object:nil];
}

#pragma mark - super method
- (void)leftButtonMethod {
    if ([MKSBCentralManager shared].connectStatus != mk_sb_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
    ...
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKSBCentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKSBDebuggerCell *cell = [MKSBDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - mk_sb_centralManagerLogDelegate
...
#pragma mark - MKSBDebuggerCellDelegate
- (void)sb_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKSBDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_sb_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_export_disableIcon.png");
    [self.startButton setTitle:@"Stop" forState:UIControlStateNormal];
    [self.contentList removeAllObjects];
    [[MKSBCentralManager shared] notifyLogData:YES];
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_sb_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKSBLogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKSBDebuggerCellModel *cellModel = self.dataList[i];
...
- (void)readDataFromLocal {
    ...
    [MKSBLogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)loadTableViewDatas:(NSArray *)logList {
    ...
    for (NSInteger i = 0; i < logList.count; i ++) {
        ...
        MKSBDebuggerCellModel *cellModel = [[MKSBDebuggerCellModel alloc] init];
...
- (void)saveDataListToLocal {
    ...
    [MKSBLogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKSBDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_delete_disableIcon.png");
...
- (void)showTipsAlert:(NSString *)msg {
    ...
    [alertView showAlertWithTitle:@"Tips!" message:msg notificationName:@"mk_sb_needDismissAlert"];
}
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKSBDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKSBDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_delete_disableIcon.png");
...
- (MKSBDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKSBDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBDebuggerController", @"sb_export_disableIcon.png");
