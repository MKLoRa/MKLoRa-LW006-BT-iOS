...
@interface MKSBAxisSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBAxisSettingDataModel *dataModel;

@end

@implementation MKSBAxisSettingController

- (void)dealloc {
    NSLog(@"MKSBAxisSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBAxisSettingController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBAxisSettingDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBAxisSettingDataModel alloc] init];
    }
    return _dataModel;
}
