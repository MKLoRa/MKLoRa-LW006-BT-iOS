...
@interface MKSBMotionModeController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBMotionModeModel *dataModel;

@end

@implementation MKSBMotionModeController

- (void)dealloc {
    NSLog(@"MKSBMotionModeController销毁");
}
...
- (void)configMotionModeEvents:(BOOL)isOn index:(NSInteger)index {
    if (index == 0) {
        ...
        self.dataModel.fixOnStationaryState = isOn;
    }else if (index == 4) {
...
- (void)loadSection9Datas {
    ...
    cellModel.isOn = self.dataModel.fixOnStationaryState;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBMotionModeController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBMotionModeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBMotionModeModel alloc] init];
    }
    return _dataModel;
}
