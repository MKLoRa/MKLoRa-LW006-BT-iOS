...
@interface MKSBTabBarController ()
...
@implementation MKSBTabBarController

- (void)dealloc {
    NSLog(@"MKSBTabBarController销毁");
...
- (void)viewDidDisappear:(BOOL)animated{
    ...
    if (![[self.navigationController viewControllers] containsObject:self]){
        [[MKSBCentralManager shared] disconnect];
    }
}
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(gotoScanPage)
                                                 name:@"mk_sb_popToRootViewControllerNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(dfuUpdateComplete)
                                                 name:@"mk_sb_centralDeallocNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(centralManagerStateChanged)
                                                 name:mk_sb_centralManagerStateChangedNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(disconnectTypeNotification:)
                                                 name:mk_sb_deviceDisconnectTypeNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceEnterDebuggerMode)
                                                 name:@"mk_sb_startDebuggerMode"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStopDebuggerMode)
                                                 name:@"mk_sb_stopDebuggerMode"
                                               object:nil];
}
...
- (void)gotoScanPage {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_sb_needResetScanDelegate:)]) {
            [self.delegate mk_sb_needResetScanDelegate:NO];
        }
    }];
}
...
- (void)dfuUpdateComplete {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_sb_needResetScanDelegate:)]) {
            [self.delegate mk_sb_needResetScanDelegate:YES];
        }
    }];
}
...
- (void)showAlertWithMsg:(NSString *)msg title:(NSString *)title{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_sb_needDismissAlert" object:nil];
    //让所有MKPickView消失
    ...
    [alertView showAlertWithTitle:title message:msg notificationName:@"mk_sb_needDismissAlert"];
}

- (void)loadSubPages {
    MKSBLoRaController *loraPage = [[MKSBLoRaController alloc] init];
    loraPage.tabBarItem.title = @"LORA";
    loraPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_lora_tabBarUnselected.png");
    loraPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_lora_tabBarSelected.png");
    MKBaseNavigationController *advNav = [[MKBaseNavigationController alloc] initWithRootViewController:loraPage];

    MKSBPositionController *positionPage = [[MKSBPositionController alloc] init];
    positionPage.tabBarItem.title = @"POSITION";
    positionPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_position_tabBarUnselected.png");
    positionPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_position_tabBarSelected.png");
    MKBaseNavigationController *positionNav = [[MKBaseNavigationController alloc] initWithRootViewController:positionPage];

    MKSBGeneralController *settingPage = [[MKSBGeneralController alloc] init];
    settingPage.tabBarItem.title = @"GENERAL";
    settingPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_setting_tabBarUnselected.png");
    settingPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_setting_tabBarSelected.png");
    MKBaseNavigationController *settingNav = [[MKBaseNavigationController alloc] initWithRootViewController:settingPage];
    
    MKSBDeviceSettingController *deviceInfo = [[MKSBDeviceSettingController alloc] init];
    deviceInfo.tabBarItem.title = @"DEVICE";
    deviceInfo.tabBarItem.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_device_tabBarUnselected.png");
    deviceInfo.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-SB", @"MKSBTabBarController", @"sb_device_tabBarSelected.png");
