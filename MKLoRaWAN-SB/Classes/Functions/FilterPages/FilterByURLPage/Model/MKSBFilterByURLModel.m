...
@interface MKSBFilterByURLModel ()
...
@implementation MKSBFilterByURLModel
...
- (BOOL)readFilterStatus {
    ...
    [MKSBInterface sb_readFilterByURLStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByURLStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readURLContent {
    __block BOOL success = NO;
    [MKSBInterface sb_readFilterByURLContentWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configURLContent {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByURLContent:self.url sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
