...
@interface MKSBFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBFilterByMacModel *dataModel;

@end

@implementation MKSBFilterByMacController

- (void)dealloc {
    NSLog(@"MKSBFilterByMacController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBFilterByMacController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBFilterByMacModel alloc] init];
    }
    return _dataModel;
}
