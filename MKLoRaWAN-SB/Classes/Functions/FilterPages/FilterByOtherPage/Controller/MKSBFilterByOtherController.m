...
@interface MKSBFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKSBFilterEditSectionHeaderViewDelegate,
MKSBFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKSBFilterByOtherModel *dataModel;

@end

@implementation MKSBFilterByOtherController

- (void)dealloc {
    NSLog(@"MKSBFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKSBFilterEditSectionHeaderView *headerView = [MKSBFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
        MKSBFilterEditSectionHeaderViewModel *model = [[MKSBFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSBFilterByRawDataCell *cell = [MKSBFilterByRawDataCell initCellWithTableView:tableView];
...
#pragma mark - MKSBFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
- (void)mk_sb_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKSBFilterByRawDataCellModel *cellModel = [[MKSBFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKSBFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)sb_rawFilterDataChanged:(mk_sb_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKSBFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_sb_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_sb_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_sb_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_sb_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKSBFilterByRawDataCellModel *cellModel = self.section1List[i];
        MKSBFilterRawAdvDataModel *model = [[MKSBFilterRawAdvDataModel alloc] init];
...
- (mk_sb_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_sb_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_sb_filterByOther_AB;
        }
        return mk_sb_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_sb_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_sb_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_sb_filterByOther_AOrBOrC;
        }
    }
    return mk_sb_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBFilterByOtherController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
