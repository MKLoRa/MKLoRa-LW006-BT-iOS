...
@interface MKSBFilterByBXPButtonModel ()
...
@implementation MKSBFilterByBXPButtonModel
...
- (BOOL)readFilterStatus {
    ...
    [MKSBInterface sb_readBXPButtonFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByBXPButtonStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readButtonAlarmContent {
    __block BOOL success = NO;
    [MKSBInterface sb_readBXPButtonAlarmFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configBXPButtonContent {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByBXPButtonAlarmStatus:self.singlePress
