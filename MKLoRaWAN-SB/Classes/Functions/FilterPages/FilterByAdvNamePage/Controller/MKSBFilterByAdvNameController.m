//
//  MKSBFilterByAdvNameController.m
...
@interface MKSBFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBFilterByAdvNameModel *dataModel;

@end

@implementation MKSBFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKSBFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKSBFilterEditSectionHeaderViewModel *headerModel = [[MKSBFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBFilterByAdvNameController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
