//
//  MKSBFilterByBXPTagController.m
...
@interface MKSBFilterByBXPTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBFilterByBXPTagModel *dataModel;

@end

@implementation MKSBFilterByBXPTagController

- (void)dealloc {
    NSLog(@"MKSBFilterByBXPTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBFilterByBXPTagController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBFilterByBXPTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBFilterByBXPTagModel alloc] init];
    }
    return _dataModel;
}
