...
@interface MKSBFilterByBXPTagModel ()
...
@implementation MKSBFilterByBXPTagModel
...
- (BOOL)readFilterStatus {
    ...
    [MKSBInterface sb_readFilterByBXPTagIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByBXPTagIDStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readFilterPreciseMatch {
    __block BOOL success = NO;
    [MKSBInterface sb_readPreciseMatchTagIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterPreciseMatch {
    __block BOOL success = NO;
    [MKSBInterface sb_configPreciseMatchTagIDStatus:self.precise sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReverseFilter {
    __block BOOL success = NO;
    [MKSBInterface sb_readReverseFilterTagIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReverseFilter {
    __block BOOL success = NO;
    [MKSBInterface sb_configReverseFilterTagIDStatus:self.reverse sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTagIDList {
    __block BOOL success = NO;
    [MKSBInterface sb_readFilterBXPTagIDListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTagIDList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterBXPTagIDList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
