...
@interface MKSBFilterByTLMController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSBFilterByTLMModel *dataModel;

@end

@implementation MKSBFilterByTLMController

- (void)dealloc {
    NSLog(@"MKSBFilterByTLMController销毁");
}
...
- (void)configFilterStatus:(BOOL)isOn {
    ...
    [MKSBInterface sb_configFilterByTLMStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configTLMVersion:(NSInteger)version {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSBInterface sb_configFilterByTLMVersion:version sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKSBFilterByTLMModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBFilterByTLMModel alloc] init];
    }
    return _dataModel;
}
