...
@interface MKSBFilterByUIDModel ()
...
@implementation MKSBFilterByUIDModel
...
- (BOOL)readFilterStatus {
    ...
    [MKSBInterface sb_readFilterByUIDStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByUIDStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNamespaceID {
    __block BOOL success = NO;
    [MKSBInterface sb_readFilterByUIDNamespaceIDWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNamespaceID {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByUIDNamespaceID:self.namespaceID sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readInstanceID {
    __block BOOL success = NO;
    [MKSBInterface sb_readFilterByUIDInstanceIDWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configInstanceID {
    __block BOOL success = NO;
    [MKSBInterface sb_configFilterByUIDInstanceID:self.instanceID sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
