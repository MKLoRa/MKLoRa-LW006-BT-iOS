//
//  MKSBFilterByRawDataController.m
//  MKLoRaWAN-SB_Example
//
//  Created by aa on 20236/29.
...
@property (nonatomic, strong)MKSBFilterByRawDataModel *dataModel;

@end

@implementation MKSBFilterByRawDataController

- (void)dealloc {
    NSLog(@"MKSBFilterByRawDataController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKSBFilterByBeaconController *vc = [[MKSBFilterByBeaconController alloc] init];
        vc.pageType = mk_sb_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKSBFilterByUIDController *vc = [[MKSBFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKSBFilterByURLController *vc = [[MKSBFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKSBFilterByTLMController *vc = [[MKSBFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //BXP-iBeacon
        MKSBFilterByBeaconController *vc = [[MKSBFilterByBeaconController alloc] init];
        vc.pageType = mk_sb_filterByBeaconPageType_bxpBeacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKSBFilterByBXPButtonController *vc = [[MKSBFilterByBXPButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP - Tag
        MKSBFilterByBXPTagController *vc = [[MKSBFilterByBXPTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 2 && indexPath.row == 3) {
        //Other
        MKSBFilterByOtherController *vc = [[MKSBFilterByOtherController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)configDeviceInfo:(BOOL)isOn {
    ...
    [MKSBInterface sb_configFilterByBXPDeviceInfoStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configBXPAcc:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSBInterface sb_configBXPAccFilterStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSBInterface sb_configBXPTHFilterStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKSBFilterByRawDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBFilterByRawDataModel alloc] init];
    }
    return _dataModel;
}
