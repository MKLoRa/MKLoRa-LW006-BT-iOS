...
@interface MKSBScanPageCellIconView : UIView
...
@implementation MKSBScanPageCellIconView
...
@property (nonatomic, strong)MKSBScanPageCellIconView *voltageView;

@end

@implementation MKSBScanPageCell

+ (MKSBScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKSBScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSBScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKSBScanPageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSBScanPageCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sb_scanCellConnectButtonPressed:)]) {
        [self.delegate sb_scanCellConnectButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKSBScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSBScanPageModel.class]) {
        return;
    }
    //顶部
...
- (UIImageView *)rssiIcon {
    if (!_rssiIcon) {
        ...
        _rssiIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBScanPageCell", @"sb_scan_rssiIcon.png");
    }
    return _rssiIcon;
}
...
- (UIImageView *)batteryIcon {
    if (!_batteryIcon) {
        ...
        _batteryIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBScanPageCell", @"sb_scan_batteryIcon.png");
    }
    return _batteryIcon;
}
...
- (MKSBScanPageCellIconView *)voltageView {
    if (!_voltageView) {
        _voltageView = [[MKSBScanPageCellIconView alloc] init];
        _voltageView.icon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBScanPageCell", @"sb_scan_voltageIcon.png");
    }
    return _voltageView;
}
