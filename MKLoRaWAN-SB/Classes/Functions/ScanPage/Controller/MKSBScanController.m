...
static NSString *const localPasswordKey = @"mk_sb_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKSBScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_sb_centralManagerScanDelegate,
MKSBScanPageCellDelegate,
...
@implementation MKSBScanController

- (void)dealloc {
    NSLog(@"MKSBScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKSBCentralManager shared] stopScan];
    [MKSBCentralManager removeFromCentralList];
}
...
- (void)viewDidLoad {
    ...
    [MKSBCentralManager shared].delegate = self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKSBScanPageCell *cell = [MKSBScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_sb_centralManagerScanDelegate
- (void)mk_sb_receiveDevice:(NSDictionary *)deviceModel {
    MKSBScanPageModel *model = [MKSBScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)refreshButtonPressed {
    if ([MKSBCentralManager shared].centralStatus != mk_sb_centralManagerStatusEnable) {
...
- (void)scanTimerRun{
    ...
    [[MKSBCentralManager shared] startScan];
...
- (void)updateDataWithScanModel:(MKSBScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKSBScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKSBScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKSBScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKSBScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKSBScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKSBScanPageModel *dataModel = self.dataList[currentIndex];
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKSBScanPageModel *)scanDataModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKSBCentralManager shared] stopScan];
    if (self.scanTimer) {
    ...
    [alertView showAlertWithTitle:@"Enter password" message:msg notificationName:@"mk_sb_needDismissAlert"];
}

- (void)connectDeviceWithDataModel:(MKSBScanPageModel *)scanDataModel {
    ...
    [[MKSBConnectModel shared] connectDevice:scanDataModel.peripheral password:(scanDataModel.needPassword ? password : @"") deviceType:scanDataModel.deviceType macAddress:scanDataModel.macAddress sucBlock:^{
...
- (void)configParams {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"sb_readRecordDataDayNumKey"];
    //读取设备存储的扫描信息页面，显示的设备总共存储了多少条的扫描数据
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"sb_recordDataTotalSumKey"];
    
    [MKSBDatabaseManager clearDataTable];
    [MKSBDatabaseManager initDataBase];
}

- (void)pushTabBarPage {
    MKSBTabBarController *vc = [[MKSBTabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBScanController", @"sb_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-SB", @"MKSBScanController", @"sb_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
