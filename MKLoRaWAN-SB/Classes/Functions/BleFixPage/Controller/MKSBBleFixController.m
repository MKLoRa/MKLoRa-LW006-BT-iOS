...
@interface MKSBBleFixController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKNormalSliderCellDelegate,
MKSBFilterRelationshipCellDelegate>
...
@property (nonatomic, strong)MKSBBleFixDataModel *dataModel;

@end

@implementation MKSBBleFixController

- (void)dealloc {
    NSLog(@"MKSBBleFixController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 4 && indexPath.row == 0) {
        //Filter by MAC
        MKSBFilterByMacController *vc = [[MKSBFilterByMacController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 4 && indexPath.row == 1) {
        //Filter by ADV Name
        MKSBFilterByAdvNameController *vc = [[MKSBFilterByAdvNameController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 4 && indexPath.row == 2) {
        //Filter by Raw Data
        MKSBFilterByRawDataController *vc = [[MKSBFilterByRawDataController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSBFilterRelationshipCell *cell = [MKSBFilterRelationshipCell initCellWithTableView:tableView];
...
#pragma mark - MKSBFilterRelationshipCellDelegate
- (void)sb_filterRelationshipChanged:(NSInteger)index dataListIndex:(NSInteger)dataListIndex {
    if (index == 0) {
        ...
        MKSBFilterRelationshipCellModel *cellModel = self.section1List[0];
    ...
    if (index == 1) {
        ...
        MKSBFilterRelationshipCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
    if (index == 2) {
        ...
        MKSBFilterRelationshipCellModel *cellModel = self.section3List[1];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)loadSection1Datas {
    MKSBFilterRelationshipCellModel *cellModel1 = [[MKSBFilterRelationshipCellModel alloc] init];
    cellModel1.index = 0;
...
- (void)loadSection3Datas {
    ...
    MKSBFilterRelationshipCellModel *cellModel2 = [[MKSBFilterRelationshipCellModel alloc] init];
    cellModel2.index = 2;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-SB", @"MKSBBleFixController", @"sb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKSBBleFixDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBBleFixDataModel alloc] init];
    }
    return _dataModel;
}
