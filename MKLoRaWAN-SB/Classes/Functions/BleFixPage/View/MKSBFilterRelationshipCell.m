...
@implementation MKSBFilterRelationshipCellModel
@end

@interface MKSBFilterRelationshipCell ()
...
@implementation MKSBFilterRelationshipCell

+ (MKSBFilterRelationshipCell *)initCellWithTableView:(UITableView *)tableView {
    MKSBFilterRelationshipCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSBFilterRelationshipCellIdenty"];
    if (!cell) {
        cell = [[MKSBFilterRelationshipCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@""];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sb_filterRelationshipChanged:dataListIndex:)]) {
            [self.delegate sb_filterRelationshipChanged:self.dataModel.index dataListIndex:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSBFilterRelationshipCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSBFilterRelationshipCellModel.class]) {
        return;
    }
