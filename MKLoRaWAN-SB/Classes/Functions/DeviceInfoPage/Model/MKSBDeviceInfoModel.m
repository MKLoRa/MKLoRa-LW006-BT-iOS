...
@implementation MKSBDeviceInfoModel
...
- (BOOL)readMacAddress {
    ...
    [MKSBInterface sb_readMacAddressWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readDeviceModel {
    __block BOOL success = NO;
    [MKSBInterface sb_readDeviceModelWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readSoftware {
    __block BOOL success = NO;
    [MKSBInterface sb_readSoftwareWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readFirmware {
    __block BOOL success = NO;
    [MKSBInterface sb_readFirmwareWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readHardware {
    __block BOOL success = NO;
    [MKSBInterface sb_readHardwareWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readManu {
    __block BOOL success = NO;
    [MKSBInterface sb_readManufacturerWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readVoltage {
    __block BOOL success = NO;
    [MKSBInterface sb_readBatteryVoltageWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
        self.voltage = [NSString stringWithFormat:@"%.3f%@",[returnData[@"result"][@"voltage"] integerValue] * 0.001,@"V"];
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
    }];
