...
@interface MKSBDeviceInfoController ()<UITableViewDelegate,
UITableViewDataSource,
MKSBTextButtonCellDelegate>
...
@property (nonatomic, strong)MKSBDeviceInfoModel *dataModel;
...
@implementation MKSBDeviceInfoController

- (void)dealloc {
    NSLog(@"MKSBDeviceInfoController销毁");
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStartDFUProcess)
                                                 name:@"mk_sb_startDfuProcessNotification"
                                               object:nil];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 5 && indexPath.row == 0) {
        //Debugger Mode
        MKSBDebuggerController *vc = [[MKSBDebuggerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSBTextButtonCell *cell = [MKSBTextButtonCell initCellWithTableView:tableView];
...
#pragma mark - MKSBTextButtonCellDelegate
/// 用户点击了右侧按钮
/// @param index cell所在序列号
- (void)sb_textButtonCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKSBUpdateController *vc = [[MKSBUpdateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)pushSelftestInterface {
    MKSBSelftestController *vc = [[MKSBSelftestController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)updateCellDatas {
    ...
    if (ValidStr(self.dataModel.firmware)) {
        MKSBTextButtonCellModel *firmwareModel = self.section1List[0];
    ...
    if (ValidStr(self.dataModel.voltage)) {
        ...
        voltage.rightMsg = self.dataModel.voltage;
    }
...
- (void)loadSection1Datas {
    MKSBTextButtonCellModel *cellModel = [[MKSBTextButtonCellModel alloc] init];
    cellModel.index = 0;
...
- (MKSBDeviceInfoModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSBDeviceInfoModel alloc] init];
    }
    return _dataModel;
}
