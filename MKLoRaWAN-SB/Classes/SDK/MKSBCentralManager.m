...
static NSString *const mk_sb_logName = @"mk_sb_bleLog";

NSString *const mk_sb_peripheralConnectStateChangedNotification = @"mk_sb_peripheralConnectStateChangedNotification";
NSString *const mk_sb_centralManagerStateChangedNotification = @"mk_sb_centralManagerStateChangedNotification";

NSString *const mk_sb_deviceDisconnectTypeNotification = @"mk_sb_deviceDisconnectTypeNotification";

static MKSBCentralManager *manager = nil;
static dispatch_once_t onceToken;

//@interface NSObject (MKSBCentralManager)
//
//@end
//
//@implementation NSObject (MKSBCentralManager)
//
//+ (void)load{
//    [MKSBCentralManager shared];
//}
//
//@end

@interface MKSBCentralManager ()
...
@property (nonatomic, assign)mk_sb_centralConnectStatus connectStatus;

@end

@implementation MKSBCentralManager

- (void)dealloc {
    [self logToLocal:@"MKSBCentralManager销毁"];
    NSLog(@"MKSBCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKSBCentralManager初始化"];
...
+ (MKSBCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKSBCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_sb_receiveDevice:)]) {
                [self.delegate mk_sb_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_sb_startScan)]) {
        [self.delegate mk_sb_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_sb_stopScan)]) {
        [self.delegate mk_sb_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_sb_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_sb_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_sb_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_sb_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_sb_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_sb_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_sb_deviceDisconnectTypeNotification
                                                            object:nil
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA04"]]) {
        ...
        if ([self.dataDelegate respondsToSelector:@selector(mk_sb_receiveStorageData:)]) {
            [self.dataDelegate mk_sb_receiveStorageData:content];
        }
        return;
    }
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA05"]]) {
        ...
        if ([self.logDelegate respondsToSelector:@selector(mk_sb_receiveLog:)]) {
            [self.logDelegate mk_sb_receiveLog:content];
        }
        return;
    }
}
...
- (mk_sb_centralManagerStatus )centralStatus {
    return ([MKBLEBaseCentralManager shared].centralStatus == MKCentralManagerStateEnable)
    ? mk_sb_centralManagerStatusEnable
    : mk_sb_centralManagerStatusUnable;
}

- (void)startScan {
    [[MKBLEBaseCentralManager shared] scanForPeripheralsWithServices:@[[CBUUID UUIDWithString:@"AA0D"]] options:nil];
}
...
- (void)addTaskWithTaskID:(mk_sb_taskOperationID)operationID
           characteristic:(CBCharacteristic *)characteristic
              commandData:(NSString *)commandData
             successBlock:(void (^)(id returnData))successBlock
             failureBlock:(void (^)(NSError *error))failureBlock {
    MKSBOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_sb_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKSBOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)addOperation:(MKSBOperation *)operation {
    if (!operation) {
        return;
    }
    [[MKBLEBaseCentralManager shared] addOperation:operation];
...
- (BOOL)notifyStorageData:(BOOL)notify {
    if (self.connectStatus != mk_sb_centralConnectStatusConnected || [MKBLEBaseCentralManager shared].peripheral == nil || [MKBLEBaseCentralManager shared].peripheral.sb_storageData == nil) {
        return NO;
    }
    [[MKBLEBaseCentralManager shared].peripheral setNotifyValue:notify
                                              forCharacteristic:[MKBLEBaseCentralManager shared].peripheral.sb_storageData];
    return YES;
}

- (BOOL)notifyLogData:(BOOL)notify {
    if (self.connectStatus != mk_sb_centralConnectStatusConnected || self.peripheral == nil || self.peripheral.sb_log == nil) {
        return NO;
    }
    [self.peripheral setNotifyValue:notify forCharacteristic:self.peripheral.sb_log];
    return YES;
}
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKSBPeripheral *trackerPeripheral = [[MKSBPeripheral alloc] initWithPeripheral:peripheral];
    [[MKBLEBaseCentralManager shared] connectDevice:trackerPeripheral sucBlock:^(CBPeripheral * _Nonnull peripheral) {
        ...
        MKBLEBase_main_safe(^{
            self.connectStatus = mk_sb_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_sb_peripheralConnectStateChangedNotification object:nil];
            if (self.sucBlock) {
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKSBOperation *operation = [[MKSBOperation alloc] initOperationWithID:mk_sb_connectPasswordOperation commandBlock:^{
        __strong typeof(self) sself = weakSelf;
        [sself saveToLogData:commandData appToDevice:YES];
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.sb_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_sb_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_sb_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKSBOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_sb_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKSBOperation <MKBLEBaseOperationProtocol>*operation = [[MKSBOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
        [sself saveToLogData:commandData appToDevice:YES];
...
- (MKSBOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_sb_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (NSDictionary *)parseModelWithRssi:(NSNumber *)rssi advDic:(NSDictionary *)advDic peripheral:(CBPeripheral *)peripheral {
    ...
    NSData *manufacturerData = advDic[@"kCBAdvDataServiceData"][[CBUUID UUIDWithString:@"AA0D"]];
    ...
    BOOL hardModule = [[content substringWithRange:NSMakeRange(2, 2)] isEqualToString:@"01"];
    ...
    NSNumber *txPower = [MKBLEBaseSDKAdopter signedHexTurnString:[content substringWithRange:NSMakeRange(16, 2)]];
    
    NSString *battery = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(18, 2)];
    NSString *voltage = [NSString stringWithFormat:@"%.3f",([MKBLEBaseSDKAdopter getDecimalWithHex:content range:NSMakeRange(20, 4)] * 0.001)];
    
    NSString *tempState = [content substringWithRange:NSMakeRange(24, 2)];
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_sb_logName dataList:@[string]];
}
