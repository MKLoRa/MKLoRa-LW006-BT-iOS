...
extern NSString *const mk_sb_peripheralConnectStateChangedNotification;

//Notification of changes in the status of the Bluetooth Center.
extern NSString *const mk_sb_centralManagerStateChangedNotification;
 ...
 */
extern NSString *const mk_sb_deviceDisconnectTypeNotification;

@class CBCentralManager,CBPeripheral;
@class MKSBOperation;

@interface MKSBCentralManager : NSObject<MKBLEBaseCentralManagerProtocol>

@property (nonatomic, weak)id <mk_sb_centralManagerScanDelegate>delegate;

@property (nonatomic, weak)id <mk_sb_storageDataDelegate>dataDelegate;

@property (nonatomic, weak)id <mk_sb_centralManagerLogDelegate>logDelegate;

/// Current connection status
@property (nonatomic, assign, readonly)mk_sb_centralConnectStatus connectStatus;

+ (MKSBCentralManager *)shared;

/// Destroy the MKSBCentralManager singleton and the MKBLEBaseCentralManager singleton. After the dfu upgrade, you need to destroy these two and then reinitialize.
+ (void)sharedDealloc;

/// Destroy the MKSBCentralManager singleton and remove the manager list of MKBLEBaseCentralManager.
