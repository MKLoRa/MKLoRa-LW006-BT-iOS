...
@implementation MKSBFilterByRawDataCellModel
...
@interface MKSBFilterByRawDataCell ()
...
@implementation MKSBFilterByRawDataCell

+ (MKSBFilterByRawDataCell *)initCellWithTableView:(UITableView *)tableView {
    MKSBFilterByRawDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSBFilterByRawDataCellIdenty"];
    if (!cell) {
        cell = [[MKSBFilterByRawDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSBFilterByRawDataCellIdenty"];
    }
    return cell;
}
...
- (void)textFieldValueChanged:(NSString *)text textType:(mk_sb_filterRawAdvDataTextType)type {
    if ([self.delegate respondsToSelector:@selector(sb_rawFilterDataChanged:index:textValue:)]) {
        [self.delegate sb_rawFilterDataChanged:type index:self.dataModel.index textValue:text];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKSBFilterByRawDataCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSBFilterByRawDataCellModel.class]) {
        return;
    }
...
- (MKTextField *)typeTextField {
    if (!_typeTextField) {
        ...
        _typeTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            [sself textFieldValueChanged:text textType:mk_sb_filterRawAdvDataTextTypeDataType];
        };
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_sb_filterRawAdvDataTextTypeMinIndex];
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_sb_filterRawAdvDataTextTypeMaxIndex];
        };
    }
...
- (MKTextField *)rawDataField {
    if (!_rawDataField) {
        ...
        _rawDataField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_sb_filterRawAdvDataTextTypeRawDataType];
        };
    }
