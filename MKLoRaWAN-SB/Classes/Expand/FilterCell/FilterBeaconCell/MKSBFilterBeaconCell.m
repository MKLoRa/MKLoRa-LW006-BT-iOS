...
@implementation MKSBFilterBeaconCellModel
@end

@interface MKSBFilterBeaconCell ()
...
@implementation MKSBFilterBeaconCell

+ (MKSBFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKSBFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSBFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKSBFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSBFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKSBFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSBFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_sb_beaconMinValueChanged:index:)]) {
                [self.delegate mk_sb_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_sb_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_sb_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
